...
app = Flask(__name__)
# Point this to the same Redis as your other workers
...
def ocr_request():
    ...
    # Don't do the work here. Instead, queue a background task.
    # The task will return the text.
    task = celery_app.send_task('ocr_worker.perform_ocr', args=[image_bytes])
    
    # Return a task ID immediately so the client can check the status later
